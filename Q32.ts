...
let new_users = ["Ahmed", "Mnzoor", "Madad", "Hadad", "ayaz"];
...
new_users.forEach(new_user =>{
    ...
    let usernametaken = current_users.some(current_user => current_user.toLowerCase() === new_userLower);   // true or fals

    // now we will apply conditions

    if(usernametaken){
        console.log(`${new_user} needs to chose a new user name because it's already be taken\n`);
